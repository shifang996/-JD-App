  ...
  <div>
    ...
    <van-row>
      <van-col class="recommendTitle">
        <img src="http://img11.360buyimg.com/jdphoto/jfs/t1/31601/22/15554/14040/5cc2a86fEbdb1098b/88174b36f85283b6.png" />
      </van-col>
    </van-row>
    <!-- 为你推荐数据渲染 -->
    <van-row type="flex" justify="center" class="shoppingcon">
      <div class="recommendshopshow" v-for="(item, index) in recommendList" :key="index" @click="details(item.gid)">
        <dt>
          <img :src="item.image" />
        </dt>
...
export default {
  ...
  methods: {
    async getFullRecommend() {
      const { data: res } = await this.$axios.get('/index/recom?token=1ec949a15fb709370f');
