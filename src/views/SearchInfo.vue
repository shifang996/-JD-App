  ...
  <div style="background-color: white;">
    ...
    <van-tabs v-model="active" @click="salesFunction" >
      <van-tab>
        <template #title>
          <van-dropdown-menu>
            <van-dropdown-item v-model="value1" :options="option1" @change="changeItem" />
          </van-dropdown-menu>
        </template>
        <template #default>
          <!-- 商品显示区域 -->
          <van-row v-if="!showRules">
            <van-col span="24">
              <van-card v-for="item in searchValListData" :key="item.cid" :thumb="item.image" :price="item.price" :cid="item.cid" :gid="item.gid" @click="turnDetailFunction(item.gid)">
                <template #title
                  ><div class="van-multi-ellipsis--l2">{{ item.title }}</div></template
                >
        ...
        <template #title>
          <van-dropdown-menu>
            <van-dropdown-item>
              ...
              <template #default>
                <van-checkbox-group v-model="checkResult" ref="checkboxGroup" @change="addSeclect">
                  <van-checkbox v-for="item in option2" :key="item.value" :name="item.value">{{ item.text }}</van-checkbox>
                </van-checkbox-group>
              </template>
            </van-dropdown-item>
          </van-dropdown-menu>
    ...
    <!-- 分页器 -->
    <van-pagination v-model="currentPageNum" :page-count="totalPageNum" force-ellipses @change="changeCurrentPageNums" />
    <!-- 遮罩层 -->
      ...
      <div class="wrapper" @click.stop>
        <div class="block">
          ...
          <van-row style="margin-top: 20px;">
            <van-col :span="12">
              <van-button style="width: 100%;" @click="Mysubmit('cancel')">取消</van-button>
            </van-col>
            <van-col :span="12">
              <van-button type="danger" style="width: 100%;" @click="Mysubmit('ok')">确定</van-button>
            </van-col>
          </van-row>
        </div>
      </div>
  ...
  methods: {
    ...
    Mysubmit(event) {
      ...
      if (event == 'ok') {
        ...
        this.searchValListData = [];
        this.searchFunction(this.$route.query.searchVal, this.option1[this.value1].rule, this.currentPageNum, this.LowPricevalue, this.HighPricevalue);
        this.filterShow = false;
      }
    },
    //跳转到商品详情页
